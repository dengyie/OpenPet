...
const createDesktopPickerArchiveManifest = ({
  ...
  errors.push(...computedSummary.errors.map((error) => `evidence: ${error}`))
  warnings.push(...computedSummary.warnings.map((warning) => `evidence: ${warning}`))
  ...
  const manifest = {
    ...
    evidence: {
      ...
      files: computedSummary.evidence.presentFiles
    },
    summary: {
