...
const FIXTURE_KEYS = new Set(['pluginPackage', 'frameFolder', 'petPack'])
const ARTIFACT_PATH_KEYS = new Set(['releaseDir', 'appPath', 'installer', 'zip', 'latestYml'])

const usage = () => [
  'Usage: node scripts/update-desktop-picker-smoke-report.js <report.json> [options]',
  '',
  'Options:',
  '  --output <report.json>          Write to a different report path instead of replacing input',
  '  --list-checks                  Print required desktop picker smoke check ids and exit',
...
const sanitizeReportPaths = (report) => {
  ...
  for (const key of FIXTURE_KEYS) {
    if (typeof report.fixture[key] === 'string') {
      report.fixture[key] = normalizeFixtureValue(key, report.fixture[key])
    }
  }
}
...
const ensureReportShape = (report) => {
  ...
  if (!isObject(report.fixture)) report.fixture = {}
...
const main = () => {
  ...
  console.log(`Desktop picker smoke report updated: ${outputPath}`)
  ...
  if (options.validateReady) {
    console.log('Desktop picker smoke report passed readiness validation.')
  } else {
