...
- Creator Studio's dashboard-first wizard is now the primary user-facing workflow for run draft, follow-up questions, confirmation, generation, QA review, retry/recover, prompt provenance, workflow guidance, and import handoff. The command entrypoints remain the automation and host-bridge entry path.
