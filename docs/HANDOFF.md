...
> Last updated: 2026-06-27 | Branch: `codex/dev`
...
- The Control Center AI pane now treats chat and image Provider configuration as the primary first-screen model-settings surface: `聊天 Provider` and `图片 Provider` open by default, secondary AI sections stay collapsed until expanded, and both Provider sections explicitly explain that local and cloud gateways share one OpenAI-compatible contract while secrets stay host-owned.
...
- Creator Studio imported follow-up routing is outcome-specific: imported action success now routes reviewers to `Actions -> Trigger Proposal Inbox`, imported action handoff failures route follow-up to `Control Center -> Plugins`, and imported pet follow-up remains `OpenPet` through `Import Approved Pet`.
