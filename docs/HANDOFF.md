...
> 当前状态：v1.0 产品化基线已完成；v1.0.1-rc.1 完成 OpenPet 改名、GitHub 仓库迁移与升级兼容；macOS 分发基线已完成，Windows 打包/CI/签名策略/冒烟证据、报告、runbook 与 collector/证据包校验/summary 工具基线已落地但尚未 release-ready
...
- 📝 Windows 桌面分发设计已记录在 [`desktop-release-design.md`](./desktop-release-design.md)，当前已补 build config、CI、平台化更新资产、签名策略护栏、冒烟证据门禁、pending 报告/runbook/collector 产物、证据包校验/summary 和报告填写工具，后续需补真实签名产物证据和冒烟验证
...
当前支持口径必须保持为：macOS release baseline complete；Windows desktop build/CI/signing-policy/smoke-evidence/reporting/runbook/collector/bundle-validation/summary baseline implemented but not release-ready；移动端不在当前范围。
...
|------|------|------|
| **功能完整性** | 95% | 所有承诺功能已实现 |
| **测试覆盖** | 226/226 ✅ | service / release 门禁覆盖 |
| **架构质量** | ⭐⭐⭐⭐⭐ | 分层清晰、安全可靠 |
| **代码质量** | ⭐⭐⭐⭐⭐ | 模块化彻底、职责单一 |
| **文档完整性** | ⭐⭐⭐⭐⭐ | 双语 README、技术文档、版本记录与发布清单完整 |
| **可发布性** | ✅ macOS RC 可发布 | Windows 打包/CI/签名策略/冒烟证据、报告、runbook 与 collector/证据包校验/summary 工具基线已落地，尚未 release-ready |
...
- [ ] Windows 桌面分发：`build/win`、`build/icon.ico`、`windows-latest` release job、平台化更新资产、签名策略护栏、冒烟证据门禁、pending 报告/runbook/collector 产物、证据包校验/summary 和报告填写工具已完成；仍需签名产物验证、安装/卸载/透明窗口冒烟验证
