...
- a landed technical full-pet atlas QA/import path backed by `source-image-validation.json` and `atlas-validation.json`; this proves packaging/import plumbing, not official-quality action rows,
...
- The current one-click full-pet policy is intentionally narrow: QA/import has base identity coverage and technical atlas validation, default full-pet generation spends no provider calls on official action row strips, and the stable shortest path is a single clean front-facing image with the saved `gpt-image-2` gateway path. The default one-click path now blocks collage or multi-view inputs instead of pretending they are a normal supported success path, and the archived Creator Workflow host smoke at `docs/release-evidence/creator-workflow-host-smoke/2026-07-04T21-56-30-104Z-main-acceptance/` proves the earlier narrowed path on `main`. Official-quality full-pet output remains open until Creator Studio generates row-specific visual strips for all nine hatch-pet rows, except approved framewise `running-right` to `running-left` mirroring.
