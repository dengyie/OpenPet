...
> Branch: `codex/extension-entry-visibility`
...
- developer-first local extension docs with `entries.commands` compatibility runtime support and explicit dashboard opening,
...
- The extension ecosystem now has two runtime-backed slices of the target model: `entries.commands` can feed the existing JavaScript compatibility runner when a package still provides `main`, and enabled plugins can explicitly open declared HTTP/HTTPS dashboards from Control Center. Wider runtime support for shell commands, services, setup, and bridge flows is still future work.
- After Phase 57, the next planning step is service lifecycle implementation, real evidence work, community extension rehearsal, a fresh whole-project review, or another high-drift service/report boundary.
...
3. Decide whether the next extension phase should implement service lifecycle controls or continue evidence/report hardening.
