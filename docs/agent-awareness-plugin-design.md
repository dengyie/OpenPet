...
| `examples/plugins/agent-awareness/web/dashboard/*` | Renders the read-only dashboard using display-time redaction, including usage, git, generated session-summary metadata, and bounded `view=details&sessionId=<sanitized-id>` focus. |
