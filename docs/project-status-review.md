...
| Release evidence | Packaged runtime evidence tooling, runtime/picker evidence-link gate, desktop picker evidence summary/archive manifests, signed release closure gate, Windows smoke/report tooling | `scripts/create-*-smoke-*`, `docs/release-evidence/` |
...
3. Fill native picker smoke evidence from launched or packaged app runs, then archive it with the desktop picker summary and archive manifest so the stricter runtime/picker archive link can point at real reviewed reports.
