...
The current one-click character-generation path is deliberately narrower than older multi-action design notes implied: full-pet QA/import proves base identity conditioning plus technical atlas packaging/import, but default full-pet generation spends no provider calls on official action row strips. The verified shortest real-user path is a single clean front-facing reference image on the saved `gpt-image-2` gateway path. The default one-click path now blocks collage or multi-view references instead of describing them as a normal supported success path, and the archived main host smoke proves both the new-pet and existing-action path with `/images/edits` reference conditioning. Official-quality full-pet output still requires state-specific generated rows for `idle`, `running-right`, `running-left`, `waving`, `jumping`, `failed`, `waiting`, `running`, and `review`, except approved framewise mirroring from generated `running-right` to `running-left`.
