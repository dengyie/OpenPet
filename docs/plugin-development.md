...
The latest archived GitHub-topic rerun at `docs/release-evidence/plugin-community-source-discovery-report/2026-07-06T15-46-14Z-openpets-model-divergence-rerun/` keeps the full current GitHub `openpets` topic plus prior intake-backed candidates in scope, the newer npm-package rerun at `docs/release-evidence/plugin-community-source-discovery-report/2026-07-06T16-17-27Z-npm-package-model-rerun/` inspects 13 published `@openpets/*` tarballs, and the direct GitHub code-search rerun at `docs/release-evidence/plugin-community-source-discovery-report/2026-07-06T16-45-00Z-github-code-search-rerun/` finds zero public current-model hits for characteristic root `plugin.json` signatures. Together they make the neighboring ecosystem split explicit: current public OpenPets docs, starter materials, published packages, and public code search are still centered on `openpets.plugin.json` or package.json-based SDK v3 layouts rather than the current OpenPet root `plugin.json` package model. A targeted invitation draft also now exists at `docs/release-evidence/plugin-community-source-invitation-kit/2026-07-06T15-46-14Z-openpets-plugin-starter-outreach/`, but the public ecosystem still has not yielded a compatible third-party current `plugin.json` package.

If an intake summary is already `ready-for-community-evidence`, prefer `create-plugin-community-source-evidence-from-intake` for the next step. It reuses the archived Phase 100 intake metadata instead of asking maintainers to manually retype archive URL, plugin path, source URL, or submitter values before Phase 99 evidence is created.
