...
- 迷你流新增**气泡本体拖拽 + 主动关闭按钮**：用户可以直接按住消息气泡或状态气泡拖离宠物，但只移动聊天框本身；一旦宠物位置变化，聊天框自动回到默认宠物锚点。
...
这一节定义的是**窗口级交互规则**，不是单纯样式细节。后续实现、测试和 review 都应以此为准。

#### 交互目标

- 用户可以只拖走 BubbleChatWindow，不拖动 pet。
- 用户可以主动关闭 BubbleChatWindow，而不是只能依赖超时隐藏或 `Esc`。
- 拖拽能力不能破坏已有的消息滚动、文本复制、输入框点击和透明区域点击穿透。

#### UI 结构

- 在迷你流顶部增加一条**细头部拖拽条**，作为唯一默认拖拽热区。
- 关闭按钮放在拖拽条右侧，按钮尺寸轻量但可稳定点击。
- 消息流区域、底部输入区、状态提示区都**不是**拖拽热区。
- 透明空白背景依旧不可点击，不因为加入头部工具条而退化成大面积命中层。

#### 拖拽规则

- 默认锚点仍由 `syncToPetWindow()` 计算，基于当前 pet bounds。
- 当用户拖动头部拖拽条时：
  - 只移动 BubbleChatWindow；
  - 不触发 pet 窗口移动；
  - 不改动 pet 的当前位置和状态。
- 拖拽结束后，BubbleChatWindow 进入“**临时手动偏移**”状态：
  - 允许当前窗口在原地继续显示；
  - 允许继续阅读、滚动、输入和关闭；
  - 不把这次偏移写成长期绝对桌面位置。
- 一旦 pet 位置发生变化，BubbleChatWindow 的临时手动偏移立即失效，并回到默认宠物锚点。
...
- `anchored`：正常跟随宠物默认锚点。
- `dragging`：用户正在拖拽头部拖拽条，此时暂停自动跟随。
- `detached-temporary`：用户刚拖拽结束，窗口暂时停留在手动位置；宠物一旦移动，立即恢复到 `anchored`。

状态迁移规则：

- `anchored -> dragging`：用户按下拖拽条并开始拖动。
- `dragging -> detached-temporary`：用户释放拖拽。
- `detached-temporary -> anchored`：检测到 pet bounds 变化、窗口重新手动打开、显式“回到宠物”动作，或后续为了简化实现的任何主动 re-anchor 事件。

#### 与现有自动跟随的优先级

当前 `PetBubbleChatWindowManager` 已有 `syncToPetWindow()`，会按宠物位置直接 `setBounds()`。新增拖拽后，必须补充以下优先级：

1. `dragging` 期间，`syncToPetWindow()` 不得覆盖用户正在拖拽的窗口位置。
2. `detached-temporary` 期间，普通内容刷新、消息追加、TTL 变化、输入框展开收起都不应把窗口强行吸回默认锚点。
3. 只有检测到 pet 自身位置变化时，才把 `detached-temporary` 恢复为 `anchored` 并重新计算默认位置。

这条优先级是为了防止出现“刚拖一点点就被自动吸回去”的糟糕手感。

#### 主动关闭规则

- 关闭按钮点击后执行的是 BubbleChatWindow 的显式隐藏，语义等同于“用户主动收起这层轻聊天”。
- 关闭只影响当前 BubbleChatWindow：
  - 不关闭 pet；
  - 不关闭完整聊天窗；
  - 不清空主会话；
  - 不清空迷你流最近消息数据。
- 关闭后允许后续新消息再次按既有 popup 规则重新出现。
- 关闭按钮应走与 `hide({ source })` 一致的主进程链路，并在日志中标注明确来源，例如 `source: 'bubble-close-button'`。

#### hit-test 与冲突边界

- 头部拖拽条和关闭按钮必须在窗口命中层中被识别为可交互区域。
- 消息滚动区域继续优先响应滚轮和文本选择，不因拖拽能力而退化。
- 输入框、发送按钮和状态文案继续优先响应输入/点击，不被误判为拖拽开始。
- 若拖拽基于 renderer 指针事件实现，需要保证：
  - `pointerdown` 只在头部拖拽条生效；
  - 拖拽过程中不会把历史消息区锁死成不可滚动状态；
  - 关闭按钮点击不会被拖拽判定吞掉。

#### 验收标准补充

- 用户可以稳定拖动轻量聊天框，pet 保持原位。
- 轻量聊天框拖走后，宠物一旦移动，聊天框自动回到默认宠物锚点。
- 新消息到来、输入框展开、气泡高度变化时，若宠物未移动，聊天框保持用户拖后的位置。
- 点击关闭按钮后，轻量聊天框立即隐藏；后续新消息仍可再次 popup。
- 头部拖拽、消息滚动、文本复制、输入框点击和关闭按钮之间无明显交互冲突。
