...
- Approval boundary: automatic execution pauses at mandatory `awaiting_identity_review` and later stops at `ready_for_review`; only a human may accept the canonical identity, approve, import, or activate
...
16. A provisionally selected canonical master requires explicit human acceptance before any action image is generated.
...
User source image
  ...
  -> provisionally selected canonical master
  -> awaiting_identity_review
  -> explicit human accept or identity retry
...
Only candidates that pass both gates are eligible. Code selects the highest overall visual score, using deterministic identity distance and then candidate ID as stable tie-breakers. This output is only the provisional master.

The run then enters mandatory `awaiting_identity_review`. The user sees the source, all eligible candidates, all rejected candidates and reasons, the proposed master, evaluator scores, and 128 by 128 previews. The user may:

- accept one eligible candidate as `canonical/master.png`;
- request a fresh identity candidate pool;
- cancel while retaining/exporting all generated identity assets.

The user cannot promote a code-QA-failing candidate through this control. No anchor grid or action Provider request may start before the accepted candidate hash is durably recorded. Quality-first production mode has no configuration that skips this checkpoint.
...
| --- | --- | --- |
| pass | code-computed pass | eligible for selection |
| fail | any/not-run | rejected |
| pass | code-computed repair/reject | rejected and eligible to inform repair |
...
`awaiting_identity_review` is a durable, mandatory state. Entering it releases the active generation lease while preserving the run. Accepting an eligible candidate records the canonical hash and resumes at `building_anchor_grids`; requesting identity retry archives the current identity revision and returns to `generating_canonical_candidates` under a newly confirmed budget snapshot.
...
- mandatory `awaiting_identity_review` blocks anchor/action generation until an eligible canonical hash is accepted;
