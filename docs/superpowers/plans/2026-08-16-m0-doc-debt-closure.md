...
**Goal:** Bring the M0 refactor documentation and task-card guidance into agreement with the verified `main` baseline and the audit recorded in #41.
...
- Preserve the verified M0 facts recorded in #41: E1-E10 evidence, E7 3/4 expected red, E6 R20 mitigation, and G11 remaining.
...
- [x] **Step 4: Refresh repo-state truth.** In 09, remove “spike not run” escape-hatch wording, update version/change history to v1.3, restore G1 to pending with a link to T34 on #41, and distinguish target G labels from gap G labels.
...
- [x] **Step 6: Review the six files for contradictory M0 claims.** Use `rg` to find `未跑`, stale branch names, `build.files`, and contradictory G1/G8 wording; resolve only claims covered by #41.
...
- [x] **Step 2: Publish README v1.6 truth.** Record M0 as complete, link E1-E10 evidence and #41, state M1-M3 remain 1/33, and list G1/G11/document debt without claiming runtime completion.
...
- [x] **Step 5: Push and open a PR against `main`.** Include #41 checkbox references and a complete HANDBACK block; do not mark any item complete until CI is green.

### Task 4: Maintain GitHub tracking after merge

**External records:**
- Update: #41 tracking through `Refs #41` and the PR review comment
- Update: #41 handoff link/comment only when the repository PR is merged

- [x] **Step 1: Post the exact scope to PR #50 and use `Refs #41`.** Distinguish repository-document changes from vault-only work.
- [ ] **Step 2: After CI and review, merge only into `main`.** Verify the merge SHA and remote CI result.
- [ ] **Step 3: Mark only the corresponding #41 checklist items complete.** Keep unresolved G1/G11/vault items unchecked.
- [ ] **Step 4: Update #41 after merge.** Add the new `main` documentation links without reintroducing hand-written progress counts.
