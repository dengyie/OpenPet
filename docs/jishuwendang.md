...
- 宠物右键菜单当前采用自定义级联菜单窗口，而不是 Electron 原生菜单。
- 一级菜单保持精简稳定：`动作`、可用时的 `和宠物聊天`、`设置`、`退出`。
- `散步` 不再占一级菜单，统一并入 `动作` 二级菜单。
- `动作` 二级菜单只显示可手动触发的动作，默认隐藏 `idle`、`working`、`waiting`、`failure` 这类状态型动作。
- 二级菜单交互规则固定为点击展开；默认优先从一级菜单右侧展开，右侧放不下时切到左侧；父子菜单作为一个 session 一起关闭。
...
1. AI Talk 仍缺更丰富的诊断、转录整形和更长周期的聊天控制能力，但当前记忆注入的相关性打分与“仅对实际注入记忆回写使用统计”的闭环已经落地。
