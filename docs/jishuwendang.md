...
> **项目状态：v1.0 产品化基线完成；当前版本 v1.0.1-rc.1 用于 OpenPet 改名与升级兼容验证；macOS 分发基线已完成，Windows 打包/CI/签名策略/冒烟证据门禁基线已落地但尚未 release-ready**
...
| Node 原生 test runner | — | 测试框架（181 个测试） |
...
- ✅ macOS 分发流程：electron-builder + GitHub Actions + 更新检查；Windows 打包/CI/平台化更新资产/签名策略护栏/冒烟证据门禁已完成，真实签名产物验证与 Windows 冒烟待补齐

**质量指标**：
- ✅ 181/181 测试通过（service / release 门禁覆盖）
- ✅ 25 个测试文件（恶意输入与 release 证据门禁测试完整）
