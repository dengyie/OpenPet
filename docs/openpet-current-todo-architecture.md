...
- Creator Studio now has deterministic official row package support beside the base-preview path: given extracted frames for all nine official Codex rows, it composes the fixed atlas, keeps unused cells transparent, writes `full-pet-row-validation.json`, and reports official real coverage only for `row-real` rows plus the approved `running-left` `approved-mirror` derivation. This does not claim real provider row generation quality; real provider evidence, contact-sheet/GIF inspection, and human visual review remain Manual-required before production art approval.
