...
- Creator Studio provider smoke evidence for the user's OpenPet development gateway is archived under `docs/release-evidence/creator-studio-provider-smoke/2026-06-28T14-06-27-403Z/`: the saved host-owned `gpt-image-2` configuration passed health check, generated one `512x512` source image in about `265s`, and passed 16-frame action QA with zero warnings under a temporary `420000ms` timeout override.
