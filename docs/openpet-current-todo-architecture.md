...
- Creator Studio's character/action generation contract is centralized in [`pet-character-generation.md`](./pet-character-generation.md). Provider-backed full-pet runs have source validation, row-generation, extraction, stable-slot, technical atlas, `row-real` / `approved-mirror`, and import-gate infrastructure, but this infrastructure is not proof of official-quality action rows; real Provider output still requires contact-sheet/GIF inspection and human visual review before production art approval. The user supplies one clean image and each Provider request carries at most one image attachment. Current orchestration requests both directional rows; the required next correction generates `running-right` once and derives `running-left` by framewise mirroring.
...
- Align full-pet orchestration with [`pet-character-generation.md`](./pet-character-generation.md): remove the independent `running-left` Provider job, derive it from an approved `running-right` row, and add a regression that proves the directional pair spends only one Provider generation request.
