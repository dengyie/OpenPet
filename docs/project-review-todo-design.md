...
| 扩展作者体验 | 已有 legacy SDK runner、示例、校验、审核包、PR/workflow 工具、developer-first local extension 文档边界、显式 setup execution、`entries.commands` compatibility runtime 支持、显式 HTTP/HTTPS dashboard opening、显式 service start/stop、手动 loopback service health check，以及 best-effort process-group cleanup | runtime 继续补齐 bridge、通用 shell command execution、硬性进程树清理保证与第三方提交演练 |
