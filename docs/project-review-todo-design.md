...
| TypeScript 边界 | 已有 IPC 常量、共享 contracts、Control Center view contracts、API facade、Control Center hooks、Pane prop surfaces 和 `typecheck` | 继续扩展到 main-process JSDoc adapters 和 high-drift service boundaries |
...
- Control Center API facade、hooks、Pane props、defaults、demo fixtures 和测试消费这些 contracts。
...
- TypeScript 覆盖最容易漂移的共享数据边界、hook 状态边界和 Pane props 边界。
