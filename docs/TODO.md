...
- [x] Complete the code and simulated-protocol path for IM Gateway Telegram AI replies. The bundled core plugin now has a permission-gated host-owned `ai:chat` bridge, pseudonymous conversation mapping, readiness-aware health, runtime credential/config mutation guards, non-blocking Telegram dispatch, 45-second bridge timeouts, fixed private/group rate limits, bounded rate tracking, reply failure handling, and a 500 external-conversation retention cap without exposing tokens or raw Telegram identifiers to renderer diagnostics.
