...
- [ ] Advance IM Gateway Phase 2 for Telegram AI replies. Start from `docs/superpowers/specs/2026-07-08-im-gateway-core-plugin-design.md`: add a permission-gated `ai:chat` service bridge route, platform/chat/user conversation mapping, IM reply delivery, reply failure handling, rate limits, and safe transcript-retention rules without exposing secrets or raw IM content to renderer/plugin config surfaces.
