...
项目使用 **Node 原生 test runner** 覆盖 service / release 路径，当前 **236 个测试全部通过**；同时维护 **Playwright Control Center UI 回归基线**，当前 8 个 UI 测试通过。
...
- ✅ Control Center shell / tab / Pet / About 冒烟覆盖、Pet / AI / Service 保存配置流程、Catalog 安装/更新流程，以及 Service MCP session 管理（8 个 Playwright 测试）
...
- ⚡ Windows 签名产物验证与冒烟验证
- ⚡ 扩展 Control Center 自动化，覆盖手动插件包安装 review
- ⚡ 更多示例插件（天气、番茄钟、RSS）
- ⚡ 插件开发教程视频
- ⚡ 用户反馈收集与迭代
