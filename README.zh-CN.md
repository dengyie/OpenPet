...
项目使用 **Node 原生 test runner** 覆盖 service / release / IPC / 示例插件路径，当前 **282 个测试全部通过**；同时维护 **Playwright Control Center UI 回归基线**，当前 9 个 UI 测试通过。
...
测试覆盖：
- ✅ Service / release / IPC / example 门禁覆盖（41 个测试文件）
- ✅ 插件提交校验、审核包与 PR packet CLI 覆盖
...
4. 运行 `npm run create-plugin-submission-report -- <plugin-dir-or-zip> --output plugin-submission-report.md` 并附上审核包
5. 运行 `npm run create-plugin-submission-pr -- <plugin-dir-or-zip> --output plugin-submission-pr.md` 并使用插件 PR 模板
6. 提交 PR 说明插件功能和权限
