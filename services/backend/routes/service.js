...
export function registerServiceRoutes(router, { manager } = {}) {
	...
	router.post("/service/token/rotate", async (ctx) => sendSuccess(ctx, await manager.rotateToken()))
	router.get("/service/logs", (ctx) => sendSuccess(ctx, manager.getLogs(ctx.query)))
