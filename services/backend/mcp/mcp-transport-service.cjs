  ...
  const handleJsonRpc = (request, body) => {
    ...
    if (body.method === 'tools/call') {
      ...
      try {
        ...
        const formatError = (error) => ({ statusCode: 200, body: createJsonRpcError(id, -32602, error.message), logPath: `/mcp/tools/call/${name || 'unknown'}` })
        const result = callTool(name, args)
        return result && typeof result.then === 'function' ? result.then(formatResult, formatError) : formatResult(result)
      } catch (error) {
