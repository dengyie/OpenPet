...
const { createMcpTransportService, MCP_PROTOCOL_VERSION } = require('./mcp-transport-service.cjs')
  ...
  const createRequestHandler = (runtime) => async (request, response) => {
    ...
    try {
      ...
      if (request.method === 'POST' && url.pathname === '/mcp') {
        ...
        const result = await runtime.mcp.handleJsonRpc(request, body)
      ...
      if (request.method === 'POST' && url.pathname === '/api/pet/say') {
        const body = await readJsonBody(request)
        const result = await petService.say({ text: body.text, ttlMs: body.ttlMs, source: 'http', sourceSurface: 'local-http' })
      ...
      if (request.method === 'POST' && url.pathname === '/api/pet/action') {
        const body = await readJsonBody(request)
        const result = await petService.playAction({ actionId: body.actionId, source: 'http' })
        respond(request, response, runtime, 200, { ok: true, result }, { path, authorized })
        return
      }
      ...
      if (request.method === 'POST' && url.pathname === '/api/pet/event') {
        const body = await readJsonBody(request)
        const result = await petService.setEvent({ ...body, source: 'http' })
        respond(request, response, runtime, 200, { ok: true, result }, { path, authorized })
        return
      }
