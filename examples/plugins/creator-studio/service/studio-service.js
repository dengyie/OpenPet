...
const { validateActionFrameQa } = require('../lib/action-frame-qa')
...
const sendError = (response, error) => {
  const statusCode = /valid JSON|too large|invalid|remaining questions|not pending|required|requires|must|missing|incomplete|does not match/i.test(error.message || '')
    ? 400
    : 500
...
const handlePost = async ({ request, response, dataDir, url }) => {
  try {
    ...
    if (approveMatch) {
      ...
      if (current.artifacts?.actionFrames) {
        validateActionFrameQa({ dataDir, actionFrames: current.artifacts.actionFrames })
      }
