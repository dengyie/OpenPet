...
const normalizeResult = ({ dataDir, result }) => {
  const normalized = {
    ...
    ...(Array.isArray(result.warningConditions) ? { warningConditions: result.warningConditions.map(String).slice(0, 16) } : {}),
    ...(result.diversityStatus === 'degraded' || result.diversityStatus === 'sufficient' ? { diversityStatus: result.diversityStatus } : {}),
    ...(result.distinctCandidateCount == null ? {} : { distinctCandidateCount: Math.max(0, Number(result.distinctCandidateCount) || 0) }),
    ...(result.evaluatedCandidateCount == null ? {} : { evaluatedCandidateCount: Math.max(0, Number(result.evaluatedCandidateCount) || 0) }),
