...
- The dashboard is read-only and focuses on sanitized session status, recent timeline, hook-plan state, aggregate usage tokens/cost/context, per-session usage metadata, git metadata, current-step summaries, generated session summaries, and diagnostics.
