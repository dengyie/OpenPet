...
test('action service persists trigger proposal inbox decisions', () => {
  let savedConfig = null
  let idIndex = 0
  const service = createActionService({
    projectRoot: '/app/openpet',
  ...
  const submittedClick = service.submitTriggerProposal({
    actionId: 'wave',
    type: 'click',
    sourcePluginId: 'openpet.creator-studio',
    sourceRunId: 'run-1',
    notes: 'User asked for click.'
  })
  const submittedState = service.submitTriggerProposal({ actionId: 'wave', type: 'state' })
  const accepted = service.acceptTriggerProposalItem(submittedClick.proposal.id)
  const rejected = service.rejectTriggerProposalItem(submittedState.proposal.id, 'Not wanted now')

  assert.equal(submittedClick.proposal.id, 'trigger-proposal-id-1')
  assert.equal(submittedState.proposal.id, 'trigger-proposal-id-2')
  ...
  assert.deepEqual(savedConfig.triggerProposalInbox.map((proposal) => ({
    ...
    type: proposal.type,
    resultCode: proposal.result?.code || ''
  })), [
    { id: 'trigger-proposal-id-2', status: 'rejected', type: 'state', resultCode: '' },
    { id: 'trigger-proposal-id-1', status: 'accepted', type: 'click', resultCode: 'applied' }
  ])
  assert.throws(
