...
test('declaration-only creator action bridge exposes action state and validation', async () => {
  const spawned = []
  const child = createFakeServiceProcess()
  const submittedTriggerProposals = []
  const actionService = {
    getPreviewConfig: () => ({
      defaultAction: 'idle',
      clickAction: 'wave',
    ...
    submitTriggerProposal: (payload) => {
      ...
      submittedTriggerProposals.push(proposal)
      return {
        proposal,
        animations: {
          defaultAction: 'idle',
  ...
  const service = createPluginService({
    ...
    pluginDirs: [createDeclarationOnlyPluginDir({
      profile: 'creator-tools',
      permissions: ['actions:read', 'actions:write', 'trigger-proposals:write']
    })],
    spawnCommandProcess: (file, args, options) => {
      spawned.push({ file, args, options })
      return child
  ...
  const submitResponse = await requestBridge(`${baseUrl}/creator/trigger-proposals/submit`, {
    method: 'POST',
    token,
    body: {
      actionId: 'wave',
      type: 'click',
      binding: 'clickAction',
      notes: 'Generated by Creator Studio.',
      sourceRunId: 'creator-run-1'
    }
  })
  ...
  assert.equal(submitResponse.body.proposal.id, 'proposal:click:wave:test')
  assert.equal(submittedTriggerProposals[0].sourcePluginId, 'weather-declaration')
  assert.equal(submittedTriggerProposals[0].sourceCommandId, 'announce')
  assert.equal(submittedTriggerProposals[0].sourceRunId, 'creator-run-1')
})

test('declaration-only trigger proposal bridge is narrower than action writes', async () => {
  const spawned = []
  const child = createFakeServiceProcess()
  const submittedTriggerProposals = []
  const actionService = {
    submitTriggerProposal: (payload) => {
      ...
      return {
        ...
        animations: {
          defaultAction: 'idle',
          clickAction: '',
          actions: [{ id: 'wave', label: 'Wave Hello', sprite: 'file:///pets/wave.png' }]
        }
      }
    }
  }
  const service = createPluginService({
    ...
    pluginDirs: [createDeclarationOnlyPluginDir({
      profile: 'creator-tools',
      permissions: ['trigger-proposals:write']
    })],
    spawnCommandProcess: (file, args, options) => {
      spawned.push({ file, args, options })
      return child
...
test('declaration-only creator asset import bridge imports package-local frame folders', async () => {
  ...
  assert.equal(actionReloadCount, 1)
