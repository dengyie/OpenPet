...
test.describe('Control Center smoke', () => {
  ...
  test('configures a custom pet hover cursor in the redesigned cursor library', async ({ page }) => {
    ...
    await expect(cursorOptionCards.first()).toHaveCSS('width', '148px')
    await expect(cursorOptionCards.first().locator('.cursor-card-preview')).toHaveCSS('min-height', '122px')
    await expect(cursorOptionCards.first().locator('img')).toHaveCSS('width', '68px')
