...
test.describe('Control Center smoke', () => {
  ...
  test('supports provider presets, model discovery, and image compatibility hints in the AI pane', async ({ page }) => {
    ...
    await expect(hub.getByTestId('ai-image-compatibility-hint')).toContainText('不透明纯色背景')
  ...
  test('shows image provider discovery results and transparency compatibility hints in the demo API', async ({ page }) => {
    ...
    await expect(page.getByTestId('image-model-compatibility')).toContainText('background=white')
    await expect(page.getByTestId('image-model-compatibility')).toContainText('本地去背')
    ...
    await expect(page.getByTestId('image-model-compatibility')).toContainText('不透明纯色背景')
