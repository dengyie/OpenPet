...
test('generateActionKeyframe retries once on soft identity-descriptor misses using the same model', async () => {
  ...
  assert.match(result.keyframe.promptRelativePath, /waving-peak-keyframe-soft-retry\.md$/)
  assert.equal(fs.existsSync(path.join(dataDir, result.keyframe.promptRelativePath)), true)
  assert.match(result.stage.promptRelativePath, /waving-peak-keyframe-soft-retry\.md$/)
